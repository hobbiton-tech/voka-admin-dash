...
export class AgentsService {
  ...
  private agentsUrl = 'https://mobicom-pilot.herokuapp.com';
  ...
  public _fetchPartners() {
    this.getPartners().subscribe((partners) => {
      this.partnersArray = this.mapPartners(partners);
  ...
  _fetchPartner(userId: string): void {
    this.getPartnerById(userId).subscribe((partners) => {
      let partner: Partner[] = this.mapPartners(partners);
  ...
  partnerTopUp(payload: any): Observable<any> {
    const url = `${this.partnersUrl}/users/update-float`;
    return this.http
  ...
  private mapPartners(partners: Partner[]) {
