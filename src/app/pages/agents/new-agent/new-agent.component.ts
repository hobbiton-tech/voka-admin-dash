...
export class NewAgentComponent implements OnInit {
   ...
   */
  validSubmit() {
    console.log('new partner submit clicked')
    ...
    try {
      ...
      if (!errors && this.submit) {
        ...
        // Submit new partner to server
        this.agentsService
          .addPartner({
            ...values,
            name: this.titleCase(values.name),
            email: values.email,
            phone_number: values.phone_number,
            password: values.password,
          })
          .pipe(take(1), catchError(this.handleError<Partner>('Partener addition')))
          .subscribe((newAgent) => {
