...
function matches(row: Transaction, term: string, pipe: PipeTransform) {
  return (
    ...
    (row.created_at !== null &&
      formatDate(row.created_at, 'd LLL yyy, h:mm:ss a', 'en-ZM')
        .toLowerCase()
        .includes(term))
  );
}
...
export class TransactionsService {
  ...
  public getTransactions(serviceId?: number): Observable<Transaction[]> {
    ...
    switch (serviceId) {
      case undefined:
        url = `${this.transactionsUrl}/transactions/all-transactions`;
        break;

      default:
        url = `${this.transactionsUrl}/transactions/all-transactions-by-serviceId/${serviceId}`;
        break;
    }
    return this.http
      .get<Transaction[]>(url)
  ...
  private getTransactionsByPartner(userId: string): Observable<Transaction[]> {
    let url: string = `${this.transactionsUrl}/transactions/all-transactions-by-user/${userId}`;
  ...
  private mapTransactions(transactions: Transaction[]) {
    return transactions.map((transaction) => {
      ...
      if (transaction.commission_amount === null) transaction.commission_amount = 0;
  ...
  private filterByDate(
    ...
    tx = tx.filter((transaction) => {
      if (
        transaction.created_at.getTime() >= dateFrom.getTime() &&
        transaction.created_at.getTime() <= dateTo.getTime()
      )
        return transaction;
    });
